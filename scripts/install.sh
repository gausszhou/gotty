...
curl -fsSL -o "$TMP/$ASSET" "$BASE_ASSET"
curl -fsSL -o "$TMP/sha256sums.txt" "$SUM_URL"
...
expected="$(awk -v n="$ASSET" '$2 == n || $2 == "*"n {print $1}' "$TMP/sha256sums.txt")"
if [ -z "$expected" ]; then
    echo "no sha256 entry for $ASSET in sha256sums.txt" >&2
    exit 1
fi
if [ "$actual" != "$expected" ]; then
    echo "checksum mismatch for $ASSET — download aborted, nothing installed" >&2
    echo "  got:      $actual" >&2
