...
- **会话即页签**：页签栏每 2 秒轮询 `GET /api/sessions`，每个活会话
  ...
  会话（历史保留，可从 ▾ 恢复）；双击页签重命名（`PUT
  /api/sessions/{id}/title` 持久化到服务端）。
...
- **延迟显示**：前端每 5 秒发送 Ping（`0x32`），收到 Pong（`0x32`）后
  ...
  （如 `12ms ● 会话1`），断开时清零。
- **会话显示名**：默认按创建顺序显示「会话1」「会话2」…（编号 = 活 +
  历史按 `created_at` 升序的位置）；**双击页签/历史项可重命名**，标题
  通过 `PUT /api/sessions/{id}/title` 持久化到服务端（活会话列表
  `GET /api/sessions` 同步返回 `title` 字段），重启后保留；清空则
  回退到自动编号。
