...
// The shell-driven half of the driver tests. They use `sh -c` because the POSIX
// shell is the natural way to make a process print, sleep and exit on demand.
// The same contracts are re-tested against `cmd /c` in driver_windows_test.go;
// what stays here is what ConPTY cannot express at all (see each test).
//
// Moved out of driver_test.go so that the Windows build is not permanently red
// on tests that hard-code /bin/sh — a path that does not exist on Windows.
...
func TestRunAnswersQueriesBack(t *testing.T) {
	...
	//
	// unix-only:Windows 上这条链路不成立 —— ConPTY 的 conhost 自己消费并应答
	// 控制台查询,查询根本到不了本进程(实测,见 driver_windows_test.go 的
	// TestConPTYConsoleHostAnswersQueriesItself)。因此在 Windows 上写一个等价
	// 用例只会测到 conhost 的行为,与本题无关。
