...
// This file holds the unix-only terminal tests. Windows has its own file
// (process_windows_test.go) driving a real ConPTY session instead.
...
//
// 只在 unix 断言:它验的是 termios 语义(PROMPT_COMMAND 注入 + 模式复位),
// Windows 上控制台模式由 ConPTY 的 console host 持有,不存在等价物。
func TestTerminal_ResetSequenceOnPrompt(t *testing.T) {
	...
	t.Cleanup(func() {
		// 强制回收:该测试不复用会话,杀掉进程后释放平台资源
		// (unix: master;windows: pseudoconsole 与管道)。
		_ = term.proc.Kill()
		_ = term.proc.Close()
	})
